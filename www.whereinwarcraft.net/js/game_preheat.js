...
const sendRequest = async (req) => {
	const res = await fetch('endpoint_preheat.php', {
		method: 'POST',
		headers: {
...
class GameState {
	...
	async startGame(isRestart = false) {
		...
		this.currentLocationMode = res.mode;
	...
	async nextRound() {
		if (this.currentLocation !== null) {
			...
			await this.panorama.setLocation(this.currentLocation, this.currentLocationMode);
...
class Panorama {
	...
	async setLocation(id, locationMode) {
		// Load the panorama for this location.
		const dir = locationMode === '0' ? 'locations_classic' : 'locations';
