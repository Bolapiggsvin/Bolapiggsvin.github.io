...
const sendRequest = async (req) => {
	const res = await fetch('endpoint_duel.php', {
		method: 'POST',
		headers: {
...
class GameState {
	...
	async startGame(isRestart = false) {
		...
		const initReq = { action: 'init' };
		initReq.token = window.forceToken;

		if (!isRestart)
			await this.ui.enterGame();
	...
	async processGuess() {
		...
		this.ui.$opponentLives.textContent = res.opponentLives;
		this.ui.$opponentScore.textContent = res.opponentScore;
		this.ui.$opponentAccuracy.textContent = Math.round(res.opponentAccuracy);
...
class UI {
	...
	_init() {
		...
		this.$opponentAccuracy = $('#game-score-opponent-accuracy-value');
		this.$opponentLives = $('#game-score-opponent-lives-value');
		this.$opponentScore = $('#game-score-opponent-score-value');
	...
	async toggleLeaderboard() {
		if (!this.isLeaderboardShown) {
			...
			if (!this.throttleLeaderboard) {
				...
				const res = await sendRequest({ action: 'leaderboard' });
...
(async () => {
	...
	const loadGame = (resumeToken = null) => {
		state.startGame(resumeToken);
	};
	...
	onButtonClick(ui.$buttonPlay, () => loadGame());
